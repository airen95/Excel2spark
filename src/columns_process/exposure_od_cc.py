...
def prd_cd_prd_sub_cd(frame):
    ...
    product_mapping = product_mapping.dropDuplicates(['Concatenated column'])
            ...
            when(col('ASSET_CLASS_MANUAL').isin(condition_in), col('ASSET_CLASS_MANUAL')).otherwise(\
                when(col('REGULATORY_RETAIL_FLAG_B4') == 'CLASS_REG_RETAIL', 'REGULATORY RETAIL').otherwise(\
                    when(col('REGULATORY_RETAIL_FLAG_B4') == 'CLASS_SME_REGULATORY_RETAIL', \
                        concat_ws("",col('REGULATORY_RETAIL_FLAG_B4'),col('EXPOSURE_SUB_TYPE_CODE'), col('ASSET_CLASS_MANUAL'))).otherwise(
                            when(col('ASSET_CLASS_FINAL').isin(condition_in1),\
                                concat_ws("",col('ASSET_CLASS_FINAL'), col('ERA_PRODUCT_TYPE'))).otherwise(
                                    concat_ws("",col('ASSET_CLASS_FINAL'), col('EXPOSURE_SUB_TYPE_CODE'), col('ASSET_CLASS_MANUAL'))
                                )
                        )
                )
    ...
    frame = frame.withColumn('PRD_SUB_CD', when(col('tmp') == 'REGULATORY RETAIL', 'REGULATORY RETAIL').otherwise(col('PRD_SUB_CD'))).drop('tmp')

# t = t.withColumn('PRD_CD', when(col('tmp')=='REGULATORY RETAIL', 'REGULATORY RETAIL').otherwise(vlookup(map_dct,'N/A')(col('tmp'))))\
#    .withColumn('PRD_SUB_CD', when(col('tmp')=='REGULATORY RETAIL', 'REGULATORY RETAIL').otherwise(vlookup(map_dct1,'N/A')(col('tmp'))))
# exposure.select('CUSTOMER_ID', 'tmp', 'PRD_CD', 'PRD_SUB_CD').where(col('CUSTOMER_ID').isin(ids)).show(50)
...
def asset_class_asset_sub_class(frame):
    ...
    assess_mapping = assess_mapping.where(col('Concatenated column')!='NaN')
...
def exposure_percent(frame):
    ...
    frame = frame.withColumn('sum1', reduce(add, [replace_null(col(x), lit(0)) for x in col2sum]))\
        ...
        .withColumn('EXPOSURE %', col('sum1')/reduce(add, (col('groupH'), col('groupI'), col('groupJ'))))
...
def netting_value_adjusted(frame):
    ...
    frame = frame.fillna({'NETTING_EXPOSURE_VALUE': 0})
